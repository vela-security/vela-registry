...
func toTab(L *lua.LState, rk *rKey, tab *lua.LTable) {
	...
	if err != nil {
		xEnv.Errorf("got registry %s stat error %v", rk.name, err)
		return
	}
	...
	if stat.ValueCount != 0 {
		...
		if e != nil {
			xEnv.Errorf("got registry %s names error %v", rk.name, e)
		} else {
			...
			for _, key := range names {
				...
				if e2 != nil {
					xEnv.Errorf("%s got value error %v", path, e2)
					continue
				}
	...
	if stat.SubKeyCount != 0 {
		...
		if e != nil {
			xEnv.Errorf("got registry %s sub names error %v", rk.name, e)
			return
		}
		...
		for _, name := range names {
			...
			if e2 != nil {
				xEnv.Errorf("got registry %s\\%s open error %v", rk.r.name, path, e2)
				continue
			}

...
func newLuaKey(key registry.Key, name string, r *registryGo) lua.LValue {
	return &rKey{value: key, name: name, r: r}
}

func (rk *rKey) String() string                         { return fmt.Sprintf("win.register.key %p", rk) }
func (rk *rKey) Type() lua.LValueType                   { return lua.LTObject }
func (rk *rKey) AssertFloat64() (float64, bool)         { return 0, false }
func (rk *rKey) AssertString() (string, bool)           { return "", false }
func (rk *rKey) AssertFunction() (*lua.LFunction, bool) { return nil, false }
func (rk *rKey) Peek() lua.LValue                       { return rk }
...
func (rk *rKey) MaxValueLen() uint32 {
	stat, err := rk.value.Stat()
	if err != nil {
		xEnv.Errorf("registry %s\\%s stat error %v", rk.r.name, rk.name, err)
		return 64
	}
